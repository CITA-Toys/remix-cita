...
class TxLogger {
  constructor (opts = {}) {
    ...
    this.logKnownTX = opts.api.editorpanel.registerCommand('knownTransaction', (args, cmds, append) => {
      ...
      var result = util.parseHexString(data.tx.content.replace('0x', ''))
      var unverifiedTransaction = Blockchain.UnverifiedTransaction.deserializeBinary(result);
      var input = unverifiedTransaction.getTransaction().getData();
      data.tx.input = util.createHexString(input)
      var el
      if (data.tx.isCall) {
