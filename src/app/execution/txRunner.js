...
const privateKey = '352416e1c910e413768c51390dfd791b414212b7b4fe6b1a18f58007fa894214'
const quota = 999999;
...
TxRunner.prototype.rawRun = function (args, cb) {
  var that = this;
  executionContext.web3().eth.getBlockNumber(function(err, res) {
    if (err) {
      console.log('get block height fail and error is: ' + err)
    } else {
      console.log('current block height is: ' + res)
      validUntilBlock = res + 100;
      run(that, args, Date.now(), cb)
    }
  })
}
...
TxRunner.prototype.execute = function (args, callback) {
  // CITA 更改此处的发交易请求参数
  console.log("TxRunner args: " + JSON.stringify(args));
  ...
  if (!executionContext.isVM()) {
    ...
    if (args.useCall) {
      ...
      executionContext.web3().eth.sendTransaction(tx, function (err, resp) {
        if (err) {
          return callback(err, resp)
        }
        tryTillResponse(resp, callback)
...
// CITA 每隔500ms调用一次getTransactionReceipt方法，直到取到交易结果数据为止
function tryTillResponse (txhash, done) {
  executionContext.web3().eth.getTransactionReceipt(txhash.hash, function (err, result) {
    if (!err && !result) {
