...
function ExecutionContext () {
  ...
  this.detectNetwork = function (callback) {
    if (this.isVM()) {
      ...
      this.web3().net.getPeerCount((err, count) => {
        console.log("peerCount: " + JSON.stringify(count));
        if (count > 0) {
          this.web3().eth.getBlockByNumber(1, (error, block) => {
            if (error) console.log('cant query first block')
            else console.log('query first block successfully')
            var id = '1'
            var name = 'Custom'
  ...
  setInterval(() => {
    if (this.getProvider() !== 'vm') {
      web3.eth.getBlockByNumber('latest', (err, block) => {
        if (!err) {
